...
- Adds visible `Planning Review` controls in `K3:K6`, with as-of month cells in `M2:N2`.
...
- `Planning Review` uses `K3:K6` for visible controls, `M2:N2` for month controls, leaves `A4` open for the main report spill, and leaves `O4:R200` open for note examples.
