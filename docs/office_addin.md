...
- Provides a standalone `Setup Asset Workflow` button for optional asset sheets. It creates/shows `Asset Hub` and `Asset Register` / `tblAssets`, applies native validation/input messages for simple asset entry, and keeps finance, semantic, evidence, and PQ backend sheets hidden; asset setup is not run from the default path.
...
That action creates the asset setup sheets and tables only when selected. It is intentionally optional and not run from the default path. The task pane color-codes the asset setup button as optional, and the standard setup completion message states that asset workflow setup is still separate. It creates and shows `Asset Register` / `tblAssets` and `Asset Hub`; it creates `Asset Setup`, `Project Asset Map`, `Semantic Assets`, `Asset Changes`, and `Asset State History` as hidden/admin workflow sheets. `tblAssets` receives native validation and input messages for `AssetID`, `AssetName`, `AssetType`, `Status`, non-negative replacement cost/useful life, and advisory `LinkedProjectID`. Rerunning it recreates the asset workflow tables from headers, so treat it as a starter/reset action on a workbook copy or before entering live asset rows. See `docs/asset_setup_workflow.md`.
...
- Optional asset setup creates visible `Asset Hub` and `Asset Register`, plus hidden/admin `Asset Setup`, `Project Asset Map`, `Semantic Assets`, `Asset Changes`, and `Asset State History` with `tblAssets`, asset staging, mapping, change, and state-history tables.
- Asset setup also writes dropdown-backed validation lists for asset type, status, condition, criticality, change type, asset state, promotion status, mapping status, and advisory relationship lists for `Asset ID` and `Project Key`. `LinkedProjectID` is advisory and allows blank or manually typed IDs.
